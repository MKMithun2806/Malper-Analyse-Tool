#!/usr/bin/env python3
“””
malper-analyse — Vulnerability report analyser & attack map generator
...
██║   ╚██████╔╝╚██████╔╝███████╗╚██████╔╝╚██████╔╝██╔╝ ██╗
╚═╝    ╚═════╝  ╚═════╝ ╚══════╝ ╚═════╝  ╚═════╝ ╚═╝  ╚═╝{W}
{DIM}  malper-analyse  ·  pentest report validator & obsidian attack mapper{W}
“””
...
ENV_FILE       = os.path.expanduser(”~/.bashrc”)   # also writes to .profile
...
if not key:
    print(f"{R}[✗]{W} No key supplied. Exiting.")
    sys.exit(1)
...
print(f”{R}[✗]{W} Unsupported file type ‘{suffix}’. Only .json and .md are accepted.”)
sys.exit(1)
if not path.exists():
print(f”{R}[✗]{W} File not found: {path}”)
sys.exit(1)
...
or whether a “critical” finding is behind layers of authentication or network controls that make it practically unexploitable.

Your job:

1. **Critically reanalyse** every finding. Downgrade severity where the scanner is clearly wrong.
1. **Remove false positives** — if a finding has no realistic exploit path, flag it as FP and exclude it from the main report.
1. **Validate CVEs / CWEs** — only include them if they are real and match the described vulnerability.
1. **Produce a clean, obsidian-compatible markdown report** with the structure described below.
1. Always think like an attacker. For each real vuln ask: “Can I actually exploit this? What’s the impact?”
...
List findings the scanner flagged that were removed or downgraded, with a one-line reason each.
...
*Report generated by [malper-analyse](https://github.com/MKMithun2806) · Model: {model}*
“””)

def call_openrouter(api_key: str, model: str, report_content: str, filename: str) -> str:
...
user_msg = f"Here is the vulnerability scanner report from file `{filename}`. Analyse it:\n\n---\n{report_content}\n---"
...
try:
    ...
    print(f"{R}[✗]{W} HTTP {e.code}: {err_body}")
    sys.exit(1)
except Exception as e:
    print(f"{R}[✗]{W} Request failed: {e}")
    sys.exit(1)

try:
    return body["choices"][0]["message"]["content"]
except (KeyError, IndexError):
    print(f"{R}[✗]{W} Unexpected response schema:\n{json.dumps(body, indent=2)}")
    sys.exit(1)
```
...
out_path = input_path.parent / f”{stem}*analysed*{ts}.md”
...
parser = argparse.ArgumentParser(
    prog="malper-analyse",
    description="Reanalyse a vulnerability scanner report and generate a clean Obsidian attack map.",
